...
KOKKOS_INLINE_FUNCTION auto  kokkos_quarkContractXX(auto cm_out,auto int i, int j, auto cm1, auto cm2){
  ...
  int init=0;
  for(int k=0;k<4;k++){
      cm_out(0,0) = init * cm_out(0,0) + cm1(1,1)*cm2(2,2)
                  - cm1(1,2)*cm2(2,1)
                  - cm1(2,1)*cm2(1,2)
                  + cm1(2,2)*cm2(1,1);
      // k1 = 1, k2 = 0
      ...
      cm_out(0,1) = init * cm_out(0,1) + cm1(2,1)*cm2(0,2)
                  - cm1(2,2)*cm2(0,1)
                  - cm1(0,1)*cm2(2,2)
                  + cm1(0,2)*cm2(2,1);
      ...
      cm_out(0,2) = init*cm_out(0,2) + cm1(0,1)*cm2(1,2)
                  - cm1(0,2)*cm2(1,1)
                  - cm1(1,1)*cm2(0,2)
                  + cm1(1,2)*cm2(0,1);
      ...
      cm_out(1,0) = init* cm_out(1,0) + cm1(1,2)*cm2(2,0)
                  - cm1(1,0)*cm2(2,2)
                  - cm1(2,2)*cm2(1,0)
                  + cm1(2,0)*cm2(1,2);
      ...
      cm_out(1,1) = init * cm_out(1,1) + cm1(2,2)*cm2(0,0)
                  - cm1(2,0)*cm2(0,2)
                  - cm1(0,2)*cm2(2,0)
                  + cm1(0,0)*cm2(2,2);
      ...
      cm_out(1,2) = init*cm_out(1,2) + cm1(0,2)*cm2(1,0)
                  - cm1(0,0)*cm2(1,2)
                  - cm1(1,2)*cm2(0,0)
                  + cm1(1,0)*cm2(0,2);
      ...
      cm_out(2,0) = init * cm_out(2,0) + cm1(1,0)*cm2(2,1)
                  - cm1(1,1)*cm2(2,0)
                  - cm1(2,0)*cm2(1,1)
                  + cm1(2,1)*cm2(1,0);
      ...
      cm_out(2,1) = init*cm_out(2,1) + cm1(2,0)*cm2(0,1)
                  - cm1(2,1)*cm2(0,0)
                  - cm1(0,0)*cm2(2,1)
                  + cm1(0,1)*cm2(2,0);
      ...
      cm_out(2,2) = init*cm_out(2,2) + cm1(0,0)*cm2(1,1)
                  - cm1(0,1)*cm2(1,0)
                  - cm1(1,0)*cm2(0,1)
                  + cm1(1,1)*cm2(0,0);
      init=1;
...
KOKKOS_INLINE_FUNCTION void kokkos_quarkContract13(int nSite, auto prop_out, auto prop1, auto prop2){


  //prop_out must be zero
  for(int j=0; j < 4; ++j){
    for(int i=0; i < 4; ++i){
     auto sout = Kokkos::subview(prop_out,nSite,i,j,Kokkos::ALL,Kokkos::ALL);
     for (int c1=0;c1<3;c1++){
       for (int c2=0;c2<3;c2++){
            sout(c1,c2) = 0;
       }
