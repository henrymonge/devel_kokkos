...
#ifndef __clover_term_qdp_4q_w_h__
...
{ 
  ...
  class QDP4QCloverTermT : public CloverTermBase<T, U>
  {
  public:
    ...
    virtual void create(Handle< FermState<T, multi1d<U>, multi1d<U> > > fs,
			const CloverFermActParams& param_,
			const QDP4QCloverTermT<T,U>& from_);
  ...
  private:
   ...
   // Empty constructor. Must use create later
  template<typename T, typename U>
  QDP4QCloverTermT<T,U>::QDP4QCloverTermT() {
  ...
  // Now copy
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::create(Handle< FermState<T,multi1d<U>,multi1d<U> > > fs,
				   const CloverFermActParams& param_,
				   const QDP4QCloverTermT<T,U>& from)
  {
    ...
    // Sanity check
    if (fbc.operator->() == 0) {
      QDPIO::cerr << "QDP4QCloverTerm: error: fbc is null" << std::endl;
      QDP_abort(1);
    }
...
#pragma omp parallel for
  ...
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::makeClov(const multi1d<U>& f, const RealT& diag_mass)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::choles(int cb)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  Double QDP4QCloverTermT<T,U>::cholesDet(int cb) const
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::ldagdlinv(LatticeREAL& tr_log_diag, int cb)
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::chlclovms(LatticeREAL& tr_log_diag, int cb)
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::triacntr(U& B, int mat, int cb) const
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  Real
  QDP4QCloverTermT<T,U>::getCloverCoeff(int mu, int nu) const 
  { 
    START_CODE();
...
#warning "Using unrolled clover term - version 2"
   ...
   */
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::apply(T& chi, const T& psi, 
  ...
  template<typename T, typename U>
  void QDP4QCloverTermT<T,U>::packForQUDA(multi1d<QUDAPackedClovSite<typename WordType<T>::Type_t> >& quda_array, int cb) const
    {
      typedef typename WordType<T>::Type_t REALT;
      int num_sites = rb[cb].siteTable().size();
