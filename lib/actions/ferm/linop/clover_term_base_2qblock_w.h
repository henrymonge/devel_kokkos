// -*- C++ -*-
/*! \file
 *  \brief Clover term linear operator for two quark product solves
 */

#ifndef __clover_term_base_2qblock_w_h__
#define __clover_term_base_2qblock_w_h__
...
{ 
  ...
  template<typename T, typename U>
	   class CloverTerm2QBBase : public DslashLinearOperator<T,
							      multi1d<U>,
							      multi1d<U> >
  {
  public:
    //! No real need for cleanup here
    virtual ~CloverTerm2QBBase() {}
  ...
  template<typename T, typename U>
  unsigned long 
  CloverTerm2QBBase<T,U>::nFlops() const {return 552;}
   ...
   */
  template<typename T, typename U>
  void CloverTerm2QBBase<T,U>::deriv(multi1d<U>& ds_u, 
			     const T& chi, const T& psi, 
			     enum PlusMinus isign) const
  {
    START_CODE();
  ...
  template<typename T, typename U>
  void CloverTerm2QBBase<T,U>::derivMultipole(multi1d<U>& ds_u, 
			     const multi1d<T>& chi, const multi1d<T>& psi, 
			     enum PlusMinus isign) const
  {
    START_CODE();
...
#undef _elem
  ...
  template<typename T, typename U>
  void CloverTerm2QBBase<T,U>::deriv_loops(const int mu, const int nu, const int cb,
				   U& ds_u_mu,
				   U& ds_u_nu,
				   const U& Lambda) const
  {
...
#warning "Derivative not implemented for 4q linop"
#if 0
...
#warning "Derivative not implemented for 4Q linop"
   ...
   */
  template<typename T, typename U>
  void CloverTerm2QBBase<T,U>::derivTrLn(multi1d<U>& ds_u, 
				 enum PlusMinus isign, int cb) const
  {
    START_CODE();
