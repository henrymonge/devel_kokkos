...
#ifndef __clover_term_force_base_w_h__
#define __clover_term_force_base_w_h__
...
{ 
  ...
  template<typename T, typename U>
	   class CloverTermForceBase : public DslashLinearOperator<T,
							      multi1d<U>,
							      multi1d<U> >
  {
  public:
    //! No real need for cleanup here
    virtual ~CloverTermForceBase() {}
    ...
    virtual void deriv(multi1d<U>& ds_u, 
	       const T& chi, const T& psi, 
	       enum PlusMinus isign) const;

    //! Take deriv of D  must be specialized to do exp-clover
    /*!
     ...
     * \return Computes   \f$chi^\dag * \dot(D} * psi\f$
     */
    virtual void derivMultipole(multi1d<U>& ds_u,
    ...
    virtual void derivTrLn(multi1d<U>& ds_u, 
		   enum PlusMinus isign, int cb) const;


    virtual void deriv_loops(const int u, const int mu, const int cb,
		     U& ds_u_mu,
		     U& ds_u_nu,
  ...
  template<typename T, typename U>
  unsigned long 
  CloverTermForceBase<T,U>::nFlops() const {return 552;}
   ...
   */
  template<typename T, typename U>
  void CloverTermForceBase<T,U>::deriv(multi1d<U>& ds_u, 
			     const T& chi, const T& psi, 
			     enum PlusMinus isign) const
  {
    START_CODE();
  ...
  template<typename T, typename U>
  void CloverTermForceBase<T,U>::derivMultipole(multi1d<U>& ds_u, 
			     const multi1d<T>& chi, const multi1d<T>& psi, 
			     enum PlusMinus isign) const
  {
    START_CODE();
...
#undef _elem
  ...
  template<typename T, typename U>
  void CloverTermForceBase<T,U>::deriv_loops(const int mu, const int nu, const int cb,
				   U& ds_u_mu,
				   U& ds_u_nu,
				   const U& Lambda) const
  {
