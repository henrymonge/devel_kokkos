...
#ifndef __clover_term_qdp_2qblock_w_h__
#define __clover_term_qdp_2qblock_w_h__

#include "state.h"
#include "util/ferm/transf.h"
#include "qdp_allocator.h"
#include "actions/ferm/fermacts/clover_fermact_params_w.h"
#include "actions/ferm/linop/clover_term_base_2qblock_w.h"
...
{ 
  ...
  class QDPCloverTerm2QBT : public CloverTerm2QBBase<T, U>
  {
  public:
    ...
    virtual void create(Handle< FermState<T, multi1d<U>, multi1d<U> > > fs,
			const CloverFermActParams& param_,
			const QDPCloverTerm2QBT<T,U>& from_);
  ...
  private:
   ...
   // Empty constructor. Must use create later
  template<typename T, typename U>
  QDPCloverTerm2QBT<T,U>::QDPCloverTerm2QBT() {
  ...
  // Now copy
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::create(Handle< FermState<T,multi1d<U>,multi1d<U> > > fs,
				   const CloverFermActParams& param_,
				   const QDPCloverTerm2QBT<T,U>& from)
  {
    ...
    // Sanity check
    if (fbc.operator->() == 0) {
      QDPIO::cerr << "QDPCloverTerm2QB: error: fbc is null" << std::endl;
      QDP_abort(1);
    }
...
#pragma omp parallel for
  ...
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::makeClov(const multi1d<U>& f, const RealT& diag_mass)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::choles(int cb)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  Double QDPCloverTerm2QBT<T,U>::cholesDet(int cb) const
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::ldagdlinv(LatticeREAL& tr_log_diag, int cb)
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::chlclovms(LatticeREAL& tr_log_diag, int cb)
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
   ...
   */
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::applySite(T& chi_in, const T& psi_in, 
  ...
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::triacntr(U& B, int mat, int cb) const
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  Real
  QDPCloverTerm2QBT<T,U>::getCloverCoeff(int mu, int nu) const 
  { 
    START_CODE();
...
#warning "Using unrolled clover term"
      // Rolled version //First D application only for NEW
#ifndef NEW
         ...
         //cchi[alpha] = diag0[alpha%(2*nc)] * ppsi[alpha]; //Only this or the next line must be done
         //cchi[alpha] = diag1[alpha%(2*nc)] * ppsi[alpha];        
...
#warning "Using unrolled clover term - version 2"
   ...
   */
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::apply(T& chi, const T& psi, 
  ...
  template<typename T, typename U>
  void QDPCloverTerm2QBT<T,U>::packForQUDA(multi1d<QUDAPackedClovSite<typename WordType<T>::Type_t> >& quda_array, int cb) const
    {
      typedef typename WordType<T>::Type_t REALT;
      int num_sites = rb[cb].siteTable().size();
  ...
  typedef QDPCloverTerm2QBT<LatticePropagator, LatticeColorMatrix> QDPCloverTerm2QB;
  typedef QDPCloverTerm2QBT<LatticePropagatorF, LatticeColorMatrixF> QDPCloverTerm2QBF;
  typedef QDPCloverTerm2QBT<LatticePropagatorD, LatticeColorMatrixD> QDPCloverTerm2QBD;
