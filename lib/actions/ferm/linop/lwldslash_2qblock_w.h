// -*- C++ -*-
/*! \file
 *  \brief Wilson Dslash linear operator for two quark product solves
 */

#ifndef __lwldslash_2qblock_h__
#define __lwldslash_2qblock_h__

#include "state.h"
#include "util/ferm/transf.h"
#include "io/aniso_io.h"
#include "actions/ferm/linop/lwldslash_base_2qblock_w.h"
...
{ 
  ...
  class QDPWilson2QBDslashT : public WilsonDslashBase<T, P, Q>
  {
  public:
    ...
    //! Full constructor
    QDPWilson2QBDslashT(Handle< FermState<T,P,Q> > state);
  ...
  //! Empty constructor
  template<typename T, typename P, typename Q>
  QDPWilson2QBDslashT<T,P,Q>::QDPWilson2QBDslashT() {}
  
  //! Full constructor
  template<typename T, typename P, typename Q>
  QDPWilson2QBDslashT<T,P,Q>::QDPWilson2QBDslashT(Handle< FermState<T,P,Q> > state)
  {
    create(state);
  }
  ...
  //! Creation routine
  template<typename T, typename P, typename Q>
  void QDPWilson2QBDslashT<T,P,Q>::create(Handle< FermState<T,P,Q> > state)
  {
    multi1d<Real> cf(Nd);
    cf = 1.0;
    create(state, cf);
			       ...
			       const multi1d<Real>& coeffs_)
  {
    ...
    if (fbc.operator->() == 0)
    {
      QDPIO::cerr << "QDPWilson2QBDslash: error: fbc is null" << std::endl;
      QDP_abort(1);
    }
  ...
  template<typename T, typename P, typename Q>
  void 
  QDPWilson2QBDslashT<T,P,Q>::apply (T& chi, const T& psi, 
...
*/
  ...
  template<typename T, typename P, typename Q>
  void 
  QDPWilson2QBDslashT<T,P,Q>::apply_D(T& chi_in, const T& psi_in, 
			  enum PlusMinus isign, int cb) const
  {
    START_CODE();
...
#error "Unsupported number of dimensions"
    ...
    QDPWilson2QBDslashT<T,P,Q>::getFermBC().modifyF(chi_in, QDP::rb[cb]);
#else
  ...
  typedef QDPWilson2QBDslashT<LatticePropagator,
			   multi1d<LatticeColorMatrix>,
			   multi1d<LatticeColorMatrix> > QDPWilson2QBDslash;


  typedef QDPWilson2QBDslashT<LatticePropagatorF,
			   multi1d<LatticeColorMatrixF>,
			   multi1d<LatticeColorMatrixF> > QDPWilson2QBDslashF;

  typedef QDPWilson2QBDslashT<LatticePropagatorD,
			   multi1d<LatticeColorMatrixD>,
			   multi1d<LatticeColorMatrixD> > QDPWilson2QBDslashD;
