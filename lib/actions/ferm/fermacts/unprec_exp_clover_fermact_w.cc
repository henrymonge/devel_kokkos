...
namespace Chroma
{
  ...
  {
    //! Callback function
    WilsonTypeFermAct<LatticeFermion, multi1d<LatticeColorMatrix>, multi1d<LatticeColorMatrix>>*
    createFermAct4D(XMLReader& xml_in, const std::string& path)
    {
      return new UnprecExpCloverFermAct(CreateFermStateEnv::reader(xml_in, path),
    ...
    //! Name to be used
    const std::string name = "UNPRECONDITIONED_EXP_CLOVER";
   ...
   */
  UnprecLinearOperator<LatticeFermion, multi1d<LatticeColorMatrix>, multi1d<LatticeColorMatrix>>*
  UnprecExpCloverFermAct::linOp(Handle<FermState<T, P, Q>> state) const
  {
    return new UnprecExpCloverLinOp(state, param);
  }
