...
class MdagMSysSolverQUDAMULTIGRIDExpClover : public MdagMSystemSolver<LatticeFermion>
{
public:
	...
	MdagMSysSolverQUDAMULTIGRIDExpClover(Handle< LinearOperator<T> > A_,
			Handle< FermState<T,Q,Q> > state_,
			const SysSolverQUDAMULTIGRIDCloverParams& invParam_) :
	  A(A_), gstate(state_), invParam(invParam_), clov(new ExpCloverTermT<T, U>() ), invclov(new ExpCloverTermT<T, U>())
	{
...
QDPIO::cout << solver_string << " init_time = "
	...
	~MdagMSysSolverQUDAMULTIGRIDExpClover()
	{
...
private:
	...
	SystemSolverResults_t qudaInvert(const ExpCloverTermT<T, U>& clover,
			const ExpCloverTermT<T, U>& inv_clov,
			const T& chi_s,
			T& psi_s
	)const;
