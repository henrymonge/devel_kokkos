...
#ifndef __syssolver_linop_quda_exp_clover_h__
#define __syssolver_linop_quda_exp_clover_h__
...
namespace Chroma
{
    ...
    namespace LinOpSysSolverQUDAExpCloverEnv
    {
    ...
    class LinOpSysSolverQUDAExpClover : public LinOpSystemSolver<LatticeFermion>
    {
        public:
             ...
             */
            LinOpSysSolverQUDAExpClover(Handle< LinearOperator<T> > A_,
                    Handle< FermState<T,Q,Q> > state_,
                    const SysSolverQUDACloverParams& invParam_) :
                A(A_), invParam(invParam_)
        {

            QDPIO::cout << "LinOpSysSolverQUDAExpClover:" << std::endl;
...
#warning "USING QUDA DEVICE IFACE"
            ...
            ExpCloverTermT<T, U> clov;
            ExpCloverTermT<T, U> invclov;
            ...
            GetMemoryPtrClover(clov.getExpOffId(),clov.getExpDiaId(),invclov.getExpOffId(),invclov.getExpDiaId());
            ...
            SystemSolverResults_t operator() (T& psi, const T& chi) const
            {
                ...
                swatch.stop();
		            ...
		            QDPIO::cout << "QUDA_"<< solver_string <<"_EXP_CLOVER_SOLVER: " << res.n_count << " iterations. Relative Rsd = " << res.resid << std::endl;
            ...
            std::vector<SystemSolverResults_t> operator() (const std::vector<std::shared_ptr<T>>& psi, const std::vector<std::shared_ptr<const T>>& chi) const override
            {
                ...
                // Check solutions
            	    ...
            	    for(int soln =0; soln < psi.size(); soln++)	{
								...
								for(int soln=0; soln < psi.size(); soln++) { 
                    QDPIO::cout << "QUDA_"<< solver_string <<"_EXP_CLOVER_SOLVER: solution " << soln << 
