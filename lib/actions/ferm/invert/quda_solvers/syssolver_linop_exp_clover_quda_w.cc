...
namespace Chroma
{
    namespace LinOpSysSolverQUDAExpCloverEnv
    {
        ...
        {
            //! Name to be used
            const std::string name("QUDA_EXP_CLOVER_INVERTER");
        ...
        {
            return new LinOpSysSolverQUDAExpClover(A, state,SysSolverQUDACloverParams(xml_in, path));
        }
    ...
    SystemSolverResults_t LinOpSysSolverQUDAExpClover::qudaInvert( const T& chi_s, T& psi_s) const {
        ...
        QDPIO::cout << "QUDA_"<<solver_string<<"_EXP_CLOVER_SOLVER: time="<< quda_inv_param.secs <<" s" ;
    ...
    void LinOpSysSolverQUDAExpClover::qudaInvertMultiSrc( const std::vector<std::shared_ptr<T>>& psi_s, 
        ...
        QDPIO::cout << "QUDA_"<<solver_string<<"_EXP_CLOVER_SOLVER: time="<< local_quda_inv_param.secs <<" s" ;
