...
#ifndef __syssolver_linop_quda_multigrid_exp_clover_h__
#define __syssolver_linop_quda_multigrid_exp_clover_h__
...
namespace Chroma
{
	...
	class LinOpSysSolverQUDAMULTIGRIDExpClover : public LinOpSystemSolver<LatticeFermion>
	{
	public:
		 ...
		 */
		LinOpSysSolverQUDAMULTIGRIDExpClover(Handle< LinearOperator<T> > A_,
				Handle< FermState<T,Q,Q> > state_,
				const SysSolverQUDAMULTIGRIDCloverParams& invParam_) :
		A(A_), is_precond( true ), invParam(invParam_), clov(new ExpCloverTermT<T, U>() ), invclov(new ExpCloverTermT<T, U>())
		{
...
//			destroyMultigridQuda(quda_inv_param.preconditioner);
	...
	private:
		...
		Handle< ExpCloverTermT<T, U> > clov;
		Handle< ExpCloverTermT<T, U> > invclov;

		SystemSolverResults_t qudaInvert(const ExpCloverTermT<T, U>& clover,
				const ExpCloverTermT<T, U>& invclov,
				const T& chi_s,
				T& psi_s
		)const;

	  void qudaInvertMultiSrc(const ExpCloverTermT<T, U>& invclov,
