      ...
      for ( int j = 0; j < 4; ++j ) {
        for ( int c1 = 0; c1  < 3; ++c1) {
          for  ( int c2 = 0; c2  < 3; ++c2 ) {
             h_adj_anti_quark_prop(n,i,j,c1,c2)=Kokkos::complex<double>( adj_anti_quark_prop.elem(qdp_index).elem(i,j).elem(c1,c2).real().elem(),adj_anti_quark_prop.elem(qdp_index).elem(i,j).elem(c1,c2).imag().elem());
             h_quark_prop1(n,i,j,c1,c2)=Kokkos::complex<double>( quark_prop_1.elem(qdp_index).elem(i,j).elem(c1,c2).real().elem(),quark_prop_1.elem(qdp_index).elem(i,j).elem(c1,c2).imag().elem());
          }
        }
      }
    }
