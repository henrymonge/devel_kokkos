...
#include "fermact.h"
#include "meas/inline/hadron/inline_propagators_2qblock_w.h"
...
{ 
  ...
  { 
    namespace
    {
      ...
      {
	return new InlinePropagators2QBlock(InlinePropagators2QBlockParams(xml_in, path));
      }
    ...
    const std::string name = "2Q_PROPAGATORS_BLOCK";
  ...
  //! Propagator input
  void read(XMLReader& xml, const std::string& path, InlinePropagators2QBlockParams::NamedObject_t& input)
  {
  ...
  //! Propagator output
  void write(XMLWriter& xml, const std::string& path, const InlinePropagators2QBlockParams::NamedObject_t& input)
  {
    push(xml, path);
  ...
  // Param stuff
  InlinePropagators2QBlockParams::InlinePropagators2QBlockParams() { frequency = 0; }

  InlinePropagators2QBlockParams::InlinePropagators2QBlockParams(XMLReader& xml_in, const std::string& path) 
  {
    try 
    {
  ...
  void
  InlinePropagators2QBlockParams::writeXML(XMLWriter& xml_out, const std::string& path) 
  {
    push(xml_out, path);
  ...
  // Function call
  void 
  InlinePropagators2QBlock::operator()(unsigned long update_no,
			       XMLWriter& xml_out) 
  {
  ...
  // Real work done here
  void 
  InlinePropagators2QBlock::func(unsigned long update_no,
			 XMLWriter& xml_out) 
  {
    START_CODE();

    QDPIO::cout << InlinePropagators2QBlockEnv::name << ": propagator calculation" << std::endl;
    ...
    {
      QDPIO::cerr << InlinePropagators2QBlockEnv::name << ": caught dynamic cast error" 
		  << std::endl;
      QDP_abort(1);
    }
    catch (const std::string& e) 
    {
      QDPIO::cerr << InlinePropagators2QBlockEnv::name << ": std::map call failed: " << e 
		  << std::endl;
      QDP_abort(1);
    }
    ...
    {
      QDPIO::cerr << InlinePropagators2QBlockEnv::name << ": error extracting source_header: " << e << std::endl;
      QDP_abort(1);
    }
    ...
    {
      QDPIO::cerr << InlinePropagators2QBlockEnv::name << ": error creating prop: " << e << std::endl;
      QDP_abort(1);
    }
    ...
    TheFermionAction2QBFactory::Instance().createObject(params.param.fermact.id,
                                   fermacttop,
	...
	Handle< FermionAction<T,P,Q> >
	  S_f(TheFermionAction2QBFactory::Instance().createObject(params.param.fermact.id,
							       fermacttop,
							       params.param.fermact.path));
	...
	success = true;
      ...
      {
	QDPIO::cout << InlinePropagators2QBlockEnv::name << ": caught exception around quarkprop: " << e << std::endl;
      }
    }
    ...
    {
      QDPIO::cerr << InlinePropagators2QBlockEnv::name << ": error extracting prop_header: " << e << std::endl;
      QDP_abort(1);
    }
    ...
    snoop.stop();
    QDPIO::cout << InlinePropagators2QBlockEnv::name << ": total time = "
		<< snoop.getTimeInSeconds() 
		<< " secs" << std::endl;

    QDPIO::cout << InlinePropagators2QBlockEnv::name << ": ran successfully" << std::endl;
